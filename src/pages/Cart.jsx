  ...
  if (items.length === 0) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-24 text-center">
        ...
        <p className="text-gray-400 mt-4">{t('cart.empty')}</p>
        <Link to="/collection"
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-12">
        ...
        {items.map((it) => (
          <div key={`${it.designId}-${it.phoneModel}`}
            ...
            <button onClick={() => removeItem(it.designId, it.phoneModel)}
              className="text-gray-300 hover:text-red-400 transition shrink-0 p-1" title={t('cart.remove')}>
      ...
      <div className="mt-8 border-t pt-6">
        ...
        {totalSaved > 0 && (
          <p className="text-xs text-gray-400 text-right mt-1">{t('cart.saved').replace('{n}', totalSaved)}</p>
        )}
        <Link to="/checkout"
